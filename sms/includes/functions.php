...
{
    // Only trust proxy headers when the immediate client is a configured proxy.
    $trusted = array_filter(array_map('trim', explode(',', TRUSTED_PROXIES)));
    $isTrusted = $trusted !== [] && in_array($_SERVER['REMOTE_ADDR'] ?? '', $trusted, true);
    if ($isTrusted) {
...
{
    $allowed = array_filter(array_map('trim', explode(',', ALLOWED_HOSTS)));
    $host = $_SERVER['HTTP_HOST'] ?? '';
    if ($allowed !== [] && !in_array($host, $allowed, true)) {
        // Reject Host-header poisoning; fall back to the canonical host.
        $host = $allowed[0];
    }
        ...
        foreach ($props as $prop => $prop_value) {
            if (!in_array($prop, $exclude)) {
                if (isset($headers[$prop])) {
                    ...
                    $header[] = ucwords(implode(" ", preg_split('/(?=[A-Z])/', $prop)));
                }
            }
        }
...
function getUserIpAddress()
{
    ...
    $isTrusted = $trusted !== [] && in_array($clientIp, $trusted, true);
    if ($isTrusted) {
 ...
 */
function getSpreadsheetReader(string $filePath): AbstractReader
{
