...
if (!empty($_GET["interval"]) && ctype_digit($_GET["interval"])) {
    ...
    array_push($where, "Message.sentDate >= ? AND Message.sentDate <= ?");
    $params[] = $start_date;
    $params[] = $end_date;
}

if (!$_SESSION["isAdmin"]) {
    array_push($where, "Message.userID = ?");
    $params[] = $_SESSION["userID"];
}

$query = appendWhere($query, $where);

$counts = MysqliDb::getInstance()->rawQueryOne($query, $params);
...
if (isset($start_date) && isset($end_date)) {
    $where[] = "Ussd.sentDate >= ? AND Ussd.sentDate <= ?";
    $ussdParams[] = $start_date;
    $ussdParams[] = $end_date;
}
if (!$_SESSION["isAdmin"]) {
    $where[] = "Ussd.userID = ?";
    $ussdParams[] = $_SESSION["userID"];
}

$ussdQuery = appendWhere($ussdQuery, $where);

$ussdCounts = MysqliDb::getInstance()->rawQueryOne($ussdQuery, $ussdParams);
