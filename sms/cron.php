...
$now = time();
$messageGroups = $db->rawQuery("SELECT DISTINCT groupID, userID, deviceID, expiryDate, prioritize FROM Message WHERE schedule <= ? AND status='Scheduled' GROUP BY groupID, userID, deviceID, expiryDate, prioritize", [$now]);
        ...
        if (empty($group["expiryDate"]) || new DateTime($group["expiryDate"]) >= new DateTime()) {
            ...
            if ($user->read()) {
                if (!is_null($user->getCredits())) {
                    $data = $db->rawQuery("SELECT number, message, type FROM Message WHERE groupID = ? AND status = 'Failed'", [$group["groupID"]]);
                    $credits = 0;
