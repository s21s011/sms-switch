        ...
        if (isset($user)) {
            if ($user) {
                if ($user->isActiveDevicesLimitReached()) {
                    ...
                    // Issue a device token for the APK to authenticate future calls.
                    ...
                    if (defined("SENDER_ID")) {
                        $response = [
                            "success" => true,
                            "data" => ["senderId" => SENDER_ID, "purchaseCode" => $purchaseCode, "token" => $token, "device" => $device],
                            "error" => null
                        ];
                    } else {
                        $response = [
                            "success" => true,
                            "data" => ["purchaseCode" => $purchaseCode, "token" => $token, "device" => $device],
                            "error" => null
                        ];
                    }
