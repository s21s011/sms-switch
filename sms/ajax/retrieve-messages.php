...
try {
    ...
    foreach ($messages as $message) {
        ...
        $messageDeviceId = $message->getDeviceID() ?? "";
        if (!isset($timeLine[$date][$message->getUserID()][$messageDeviceId]["messages"])) {
            $timeLine[$date][$message->getUserID()][$messageDeviceId]["messages"] = [];
            if (!isset($_POST["device"]) || $_POST["device"] === 'null') {
                $device = $data[$message->getUserID()][$messageDeviceId];
                $link = "messages.php?device={$messageDeviceId}";
                if ($_SESSION["isAdmin"]) {
                    ...
                    $link = "messages.php?user={$message->getUserID()}&device={$messageDeviceId}";
                }
                $timeLine[$date][$message->getUserID()][$messageDeviceId]["header"] = ["title" => $device, "link" => $link];
            }
        }
        ...
        if (!is_null($message->getSimSlot())) {
            if (isset($sims[$messageDeviceId][$message->getSimSlot()])) {
                $timeLineItem["footer"][] = $sims[$messageDeviceId][$message->getSimSlot()];
            } else {
        ...
        $timeLine[$date][$message->getUserID()][$messageDeviceId]["messages"][] = $timeLineItem;
    }
